r"""Meta-learning optimizers — MAML, FOMAML, and Reptile.

## Formalisms

A standard learning problem optimises parameters $\theta$ over a single
dataset $D$ and loss surface $L$:

$$\min_\theta \; \frac{1}{n} \sum_i L(f_\theta, D_i)$$

Meta-learning extends this to a *distribution of tasks*.  Each task
$\mathcal{T}_i = (D_i^s, D_i^q)$ has a support set (for adaptation) and a
query set (for evaluation).  The meta-objective over a batch of $N$ tasks:

$$\min_\theta \; \frac{1}{N} \sum_i w_i \cdot L(f_{\theta'_i}, D_i^q)$$

where $w_i$ are per-task weights (uniform by default) and $\theta'_i$ is the
result of $k$ inner gradient steps on the support set:

$$
\theta_i^{(0)} = \theta, \qquad
\theta_i^{(j+1)} = \theta_i^{(j)} - \alpha \cdot T\bigl(\nabla L(f_{\theta_i^{(j)}}, D_i^s)\bigr), \qquad
\theta'_i = \theta_i^{(k)}
$$
...
- **MAML**: $g = \nabla_\theta \, \frac{1}{N} \sum_i w_i \, L(f_{\theta'_i}, D_i^q)$ —
  full backprop through inner steps (second-order).
- **FOMAML**: $g = \frac{1}{N} \sum_i w_i \, \nabla_{\theta'_i} L(f_{\theta'_i}, D_i^q)$ —
  treats adapted params as leaves (first-order approximation).
- **Reptile**: $g = \frac{1}{N} \sum_i (\theta'_i - \theta)$ and
  $\theta \leftarrow \theta + \beta \cdot g$ — parameter interpolation, no
  query set needed.
...
| Math | Code |
|---|---|
| $\theta$ | ``model.parameters()`` at context entry |
...
| $\theta'_i = \mathrm{Adapt}(\ldots)$ | ``ms.task(...) -> AdaptedState`` |
| $L(f_{\theta'_i}, D^q)$ | query loss (computed internally) |
...
- ``inner_step_fn`` plugs in differentiable optimizers (see ``sgd()``,
  ``mutation_optimizer()``).
- ``inner_reg_fn`` adds regularisation to the inner loss (e.g. L2 toward
  $\theta$, KL divergence, elastic weight consolidation).
...
The context manager is sugar over the builder — it calls ``MetaStep()`` on
enter and ``.step()`` on exit.  Same object, same capabilities.
